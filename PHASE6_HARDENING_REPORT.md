...
- **CI**: `.github/workflows/ci.yml` extended with `Database authorization harness (H-3, PGlite)` step (`node scripts/db-authz-attack.mjs`) and secret/audit gates (M-7). Added `@electric-sql/pglite` dev dep (0 vuln).
- **Final validation (2026-09-06)**: the step originally shipped as `continue-on-error: true` on PGlite `^0.3.9`, which cannot load `pgcrypto` and so never executed a single policy. PGlite is now pinned exactly to `0.5.8` (harness 91/91 locally) and the step is a **blocking** gate. Detector validated by mutation: opening `files_select_client` to `using (true)` fails 2/91 checks (exit 1). See `PHASE6_FINAL_VALIDATION.md`.
