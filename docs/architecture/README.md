...
The full Phase 4 design — role matrix, resource/action matrix, tenant isolation,
project-level access, client visibility, internal-only data, RLS strategy,
server/API/frontend strategies — is [authorization.md](authorization.md), with
[ADR-0007](adr/ADR-0007-two-authorization-layers.md) (the two layers enforce
**different** questions, never the same rule twice) and
[ADR-0010](adr/ADR-0010-four-roles-r1-accepted-and-open.md) (four roles; R-1
accepted and left open).

Two layers, both mandatory, and — the correction Phase 4 made to the original
plan — **not enforcing the same rules**. The capability matrix answers _"may
this actor attempt this verb?"_; RLS answers _"which rows may this statement
touch?"_. Nothing is encoded twice, so no pair of artefacts can silently
disagree; the one fact both need (identity, status, tenant reach) comes from the
same `SECURITY DEFINER` helpers.
...
> ⚠️ **Risk R-1 — decided in Phase 4, and deliberately still open.** The four
> specified roles cannot express "internal staff scoped to their own team and
> assigned engagements", so every specialist across seven teams holds
> cross-tenant `ADMIN`. Phase 4's decision
> ([ADR-0010](adr/ADR-0010-four-roles-r1-accepted-and-open.md)) is to **stay at
> four roles and accept the risk**, rather than add a permanent enum value on
> the designer's authority. The recommendation to add `TEAM_MEMBER` stands and
> is now costed: one enum value, one matrix column, one qualifier flip, one
> policy predicate. The blast radius is reduced where it can be without a new
> role (`platform_grant:read` is SUPER_ADMIN-only, ADMIN cannot alter a
> SUPER_ADMIN account, reopening transitions are SUPER_ADMIN-only, MFA
> mandatory, cross-tenant reads audited). `src/lib/domain/roles.ts` documents
> the gap on `ROLES`, and `tests/unit/domain.spec.ts` keeps the tripwire armed
> in both directions. Recorded and enforced, not resolved unilaterally.
...
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R-1  | No non-privileged internal role: all internal staff need cross-tenant `ADMIN`                                                                                                                                                                                 | 🔴  | **Owner decision required.** Phase 4 decided to stay at four roles and **accept** this risk ([ADR-0010](adr/ADR-0010-four-roles-r1-accepted-and-open.md)) rather than add a permanent enum value unilaterally — so the row stays open. Documented on `ROLES` in `src/lib/domain/roles.ts` and enforced by a tripwire test in `tests/unit/domain.spec.ts`, which fails if a fifth role appears while this row still says "Owner decision required"; the Phase 4 permission layer is authored so a fifth role is one enum value + one matrix column + one qualifier flip + one policy predicate. Blast radius reduced meanwhile: `platform_grant:read` is SUPER_ADMIN-only, ADMIN cannot alter a SUPER_ADMIN account, reopening transitions are SUPER_ADMIN-only, MFA mandatory, cross-tenant reads audited |
...
| R-19 | **`can_access_storage_path()` checks tenancy only** — used alone for `storage.objects` SELECT it would let any client fetch every object under their own organization's prefix, including internal working files (the flag lives on `files`, not on the path) | 🟠  | Found during the Phase 4 design, before any policy was written. `authorization.md` §H.6 adds `can_read_storage_object()`, which joins `files` for `client_visible` + `scan_status = 'CLEAN'`; pgTAP asserts the path check and the metadata row never disagree                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| R-20 | **`profiles` has no column-level grants**, so a legitimate client read of a co-member or of staff on their account also exposes `phone`, `last_seen_at` and `mfa_enrolled_at`                                                                                 | 🟠  | Found during the Phase 4 design. `authorization.md` §F.1 applies the same revoke-and-re-grant treatment the commercial tables already have, leaving clients `full_name`, `display_name`, `avatar_path` and `job_title`. Same treatment extended to `project_memberships.allocation_pct` and `invitations.token_hash`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| R-21 | The capability matrix and the RLS policy set encode different halves of one intent and could drift apart                                                                                                                                                      | 🟠  | ADR-0007 keeps them non-overlapping by construction; `tests/unit/permissions.spec.ts` invariant 6 fails the build when a matrix cell implies a policy that does not exist, and pgTAP proves the app-side tenant mirror equals `has_org_access()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
